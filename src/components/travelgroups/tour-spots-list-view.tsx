...
export default function TourSpotsListView({ tourSpotList = [], scDate }: { tourSpotList: any[], scDate: number }) {
  ...
  return (
    <div className="tour-spots-container">
      ...
      <button
        id="bottom-button-postion-time" // 이 id를 추가
        className={`${selectedSpots.size > 0 ? 'long-active-button' : 'long-nomal-button'} travelgroups-font-size`}
        onClick={selectedSpots.size > 0 ? handleAddButtonClick : undefined}
      >
        추가
      </button>
